...
<?php 
    ...
    if(isset($_POST['Login'])){
        ...
        $data=$x->VerifyLogin();
          if ($data){
            $_SESSION['customer_email']              = $x->vendor_email; 
            foreach($data as $value){
            $_SESSION['customer_id']                =$value['customer_id'];
            
                    }
                 foreach($data as $value){
                $_SESSION['customer_email']=$value['customer_email'];
                $_SESSION['customer_password']=$value['customer_password'];
...
<form action="" method="post" enctype="multipart/form-data">
  ...
  <h1 style="color:#000">Customer Login</h1>
